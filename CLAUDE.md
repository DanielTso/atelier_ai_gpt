...
**Supabase Postgres** via Drizzle on `postgres-js` (`drizzle-orm/postgres-js`). Connection at `src/db/index.ts`: `postgres(DATABASE_URL, { prepare: false, max: 10, idle_timeout: 20, connect_timeout: 10, max_lifetime: 60 * 30 })` — `prepare: false` is **required** for Supabase's transaction pooler (no prepared statements); the pool bounds/timeouts (seconds) were added in v4.51.0, and a missing `DATABASE_URL` now throws at startup. Use the **pooled** URL (`DATABASE_URL`, port 6543) at runtime and the **direct** URL (`DIRECT_URL`, port 5432) for migrations.
...
- **`documents.extraction_method`** — text column (`'text'` | `'vision'` | `'hybrid'`), nullable; added by migration `drizzle/0004_sudden_ben_grimm.sql`. Set by `/api/documents/process`; surfaced in `GET /api/documents` and displayed as a badge in `DocumentCard`.
- **`documents.failed_pages`** — `jsonb`, nullable (array of absolute page numbers, `null` when none); added by migration `drizzle/0015_chemical_sugar_man.sql`. Written by `extractViaVision`/`extractPagesViaVision` when a segment fails after retries or a page is skipped as oversize; plumbed through `/api/documents/process` → `GET /api/documents` (`failedPages`) → `DocumentCard`'s Partial badge tooltip (`formatPageList` in `src/lib/utils.ts` renders run-length ranges, e.g. "12–14, 30") and to `read_document`'s `unavailablePages`. No backfill — existing docs gain it on re-upload.
...
- **`document_chunks.content_tsv`** — generated `tsvector` column (`GENERATED ALWAYS AS (to_tsvector('english', content)) STORED`) + GIN index `idx_chunks_tsv`, plus a GIN trigram index `idx_chunks_trgm` on `document_chunks.content` (`pg_trgm` extension). Raw-SQL migration `drizzle/0016_hybrid_search.sql` — **not represented in `src/db/schema.ts`** (Drizzle doesn't model generated columns); backs `findChunksByKeyword` (`src/lib/keywordSearch.ts`), the keyword leg of hybrid retrieval (see Context Pipeline).
- **Migrations** (versioned, in `drizzle/`): `npx drizzle-kit generate` to author, `DIRECT_URL=... npx drizzle-kit migrate` to apply. `drizzle.config.ts` uses `dialect: "postgresql"`. (Legacy `npx drizzle-kit push` is no longer the workflow.) Applied to Supabase: `0000`–`0016` (latest: `0015` `failed_pages`, `0016` `pg_trgm`/`content_tsv` hybrid search — both applied 2026-07-12, before the v4.48.0 deploy). **Standing rule for schema-adding releases: migrate BEFORE pushing/deploying.** Deployed Drizzle code emits explicit column lists, so an unmigrated DB breaks whole tables' queries app-wide (e.g. a missing `failed_pages` would break all of `GET/POST /api/documents*` — listing, uploads, the Files panel), not just the new feature.
...
3. **API Routes**:
   - `POST /api/chat` — Streams LLM responses. Claude models (`claude-*`) route to Anthropic with **web search** enabled (`anthropic.tools.webSearch_20250305`). The Gemini image model (`*image*`) gets `responseModalities: ['TEXT', 'IMAGE']`. Applies five-layer context (see below). Default model fallback is `claude-opus-4-8`. For Claude project chats with ≥1 `ready` document, also wires the **`read_document`** tool (whole-document mode — see Context Pipeline) plus a `[Project documents]` manifest and `READ_DOCUMENT_GUIDANCE` in the system prompt.
   ...
   - **Document upload — 3-step direct-to-Storage flow (Phase C-storage):**
     ...
     - `POST /api/documents/process` `{ documentId }` — downloads original from Storage, runs the C2 extract pipeline (text / thin-PDF vision fallback / image vision), uploads a WebP thumbnail, chunks + embeds, records `extraction_method` (`'text'` or `'vision'`), sets status `processing` → `ready | error`. The old inline `POST /api/documents` + `createDocument` action are retired. **Replace path (v4.51.0):** a TOTAL embed failure aborts BEFORE any destructive write — the previous revision's chunk index stays active, status returns to `ready`, response is 502 (new `rev<N>` storage objects orphan by design; a retry overwrites the same paths).
...
- `src/components/chat/sidebar/` — Decomposed sidebar: `Sidebar.tsx` (orchestrator — renders `SidebarHeader`, `SidebarNav`, `RecentsSection`, `SidebarFooter`, `CollapsedSidebar`), `types.ts` + `SidebarActionsContext.tsx` (shared types/context), `ChatItem`, `SmartChatMenu`. (The pre-decomposition section components `QuickChatsSection`/`ProjectsSection`/`ArchivedSection`/`ProjectItem` + `useCollapseState` were dead and removed in v4.51.0.)
...
- `src/hooks/` — Custom hooks (useLocalStorage, usePersonas, useAppearanceSettings, etc.). **`useDialogs`** (the 10 dialog flags + 6 targets behind a per-dialog controller) and **`useChatPersistence`** (the `useChat` `onFinish` pipeline — save message/images/attachments, embed, persona usage, summarize, auto-memory, title, artifact re-fetch; unit-tested) were extracted from `page.tsx` in v4.33.0; **`useWebIngest`** drives "Add from web" (map + per-page ingest); **`useUrlNavSync`** (v4.49.0) mirrors nav state (`activeView`/`activeProjectId`/`activeChatId`) into the URL via the native History API — browser back/forward walk in-app views, deep links/refresh restore; the URL scheme lives in `src/lib/navState.ts` (pure `parseNavUrl`/`navToUrl`: `?view=`, `?project=`, `?chat=`); `useSearchParams` is deliberately avoided (it would force a Suspense boundary around the single-page app). Dialogs/artifact panel/lightboxes are intentionally NOT in history.
...
- `src/lib/` — Utilities: `settings.ts` (DB-first/env-fallback config), `embeddings.ts` (pgvector search), `retrieval.ts` (RAG pipeline orchestrator, incl. hybrid RRF fusion), `ragConfig.ts` (tunable RAG knobs), `queryRewrite.ts` (conversational query rewrite), `rerank.ts` (LLM rerank), `mmr.ts` (diversity selection), `chunking.ts` (document chunker), `keywordSearch.ts` (RAG Phase 3 — `findChunksByKeyword`: Postgres FTS over `content_tsv` + trigram `ILIKE` on identifier tokens like `SW-101`; `identifierTokens`), `rrf.ts` (RAG Phase 3 — `rrfFuse`: Reciprocal Rank Fusion merging ranked candidate lists by `1/(k+rank+1)`), `chatStage.ts` (pure stage machine driving `ThinkingStatus` copy from stream parts — `reading-documents` maps an active `read_document` tool part), `fileAttachments.ts` (image/file handling), `providers.ts` (shared AI provider factory), `fileExtraction.ts` (shared file parsing), `errors.ts` (API error helper), `validation.ts` (Zod request schemas), `storage.ts` (server-only Supabase Storage wrapper — `isStorageConfigured`, `createSignedUploadUrl`, `uploadBuffer`, `downloadToBuffer`, `createSignedDownloadUrl`, **`createSignedDownloadUrls`/`signedArtifactUrls`** (batch — one `createSignedUrls` request signs many vs N round-trips; HTML split for download-disposition; used by `getAllArtifacts`/`getGeneratedImages`/`getChatAttachments`/documents route), `removeObjects`; uses service-role key), `thumbnails.ts` (`generatePdfThumbnail` + `generateImageThumbnail` → WebP via `@napi-rs/canvas`, best-effort), `storageClient.ts` (browser-side anon-key Supabase client for `uploadToSignedUrl`), `tavily.ts` (server-only Tavily `mapSite`/`extractUrl` wrapper — web ingestion, key via `getTavilyApiKey`), `ingest.ts` (shared `ingestText` tail: chunk→save→embed→status, used by both `documents/process` and `documents/web-ingest`), `utils.ts` (incl. `formatPageList` — run-length page-range formatting for the Partial badge tooltip), `navState.ts` (v4.49.0 — pure URL⇄NavState mapping for browser-history sync; consumed by `useUrlNavSync`), `highlighter.ts` (v4.50.0 — lazy client-side shiki v4 singleton, `codeToHtmlSafe`/`resolveShikiLang`, dual vitesse themes, grammars loaded on demand; used by chat `CodeBlock` + code `ArtifactPreview`)
- `src/lib/documents/` — RAG Phase 3 whole-document mode: `tool.ts` (`createReadDocumentTool` — the AI SDK v6 `tool()` for `read_document`; reads a project document's stored `extracted.txt` from Storage window-by-window, project-scoped, in-band errors on missing text, surfaces `unavailablePages` from `failed_pages`) and `windowing.ts` (`sliceWindow` — cuts on `# Page n` anchors when present, falls back to character-offset windows; capped at `READ_DOC_WINDOW_CHARS`, default `100000` chars ≈ 25k tokens).
- `src/lib/artifacts/` — Artifact engine: `types.ts` (`ArtifactType` = `xlsx|docx|pdf|pptx|html|code`, `SheetSpec`, `RenderedArtifact` — `ext` is a string: the type itself for documents, the language extension for code); `code.ts` (v4.50.0 — `CODE_LANGUAGES` registry: id/label/ext/shikiLang for python·bash·typescript·javascript·sql·json·yaml·markdown·powershell; a code artifact's LANGUAGE persists in the `format` column, `type='code'` `format='python'` — no migration); `style.ts` (Atelier brand palette + per-library color/font helpers); `markdown.ts` (`parseMarkdown` → neutral AST via the `marked` lexer, plus `mdToPlainText`); `toXlsx.ts`/`toDocx.ts`/`toPdf.ts`/`toPptx.ts` (brand-styled renderers — styled headers, real tables, lists, page numbers; xlsx cells run through `mdToPlainText` so raw Markdown never leaks); HTML and code need no renderer (the model's string IS the file — `text/html` / `text/plain`); `render.ts` (`renderArtifact(type, title, content, language?)` dispatch); `tool.ts` — the AI SDK v6 `tool()` for `generate_artifact`. Claude calls it with `{ type, title, format, content, language? }` (`format` = `markdown|sheets|html|code`; `language` required for code); `execute` renders the file, uploads to `artifacts/<projectId|standalone>/<id>/<slug>.<ext>`, persists an `artifacts` row (+ seeds version 1), and returns `{ artifactId, title, type, downloadUrl }` (signed URL TTL `ARTIFACT_URL_TTL_SECONDS` = 24h). The tool is **chat-first**: only generate a file on an explicit file/web-page request, otherwise answer in chat. Wired into `/api/chat` for Claude when a `chatId` exists and Storage is configured.
...
**Advanced retrieval pipeline (Phase B2, hybrid-fused in RAG Phase 3)** — `src/lib/retrieval.ts` `retrieveContext()` orchestrates: **query-rewrite** (`queryRewrite.ts`, Gemini Flash → standalone query) → embed → **vector top-N** (default 20) **∥ keyword top-N** (`keywordSearch.ts` `findChunksByKeyword`, documents path only) → **RRF fusion** (`rrf.ts` `rrfFuse`, documents path only — merges both ranked lists by `score = Σ 1/(RRF_K + rank + 1)`; keyword-only hits with no embedding survive the MMR step below as exact matches, including in the no-rerank path) → **MMR** diversity (`mmr.ts`, λ 0.7) → **LLM rerank** (`rerank.ts`, Gemini Flash) → top-k (docs 3 / msgs 5). Every stage is **best-effort and degrades to the prior stage** (with all toggles off it's plain vector top-k; a keyword-search failure logs and degrades to vector-only). All knobs live in `src/lib/ragConfig.ts` (`getRagConfig()`) with env overrides: `RAG_DOC_THRESHOLD`, `RAG_MSG_THRESHOLD`, `RAG_TOP_N`, `RAG_DOC_TOP_K`, `RAG_MSG_TOP_K`, `RAG_MMR_LAMBDA`, `RAG_REWRITE_ENABLED`, `RAG_RERANK_ENABLED`, `RAG_MMR_ENABLED`, `RAG_HYBRID_ENABLED` (default `true`), `RAG_RRF_K` (default `60`), `RAG_KEYWORD_TOP_N` (default = `RAG_TOP_N`). Hybrid scope is **document chunks only** — message-side retrieval stays vector-only (no observed failure case). Note: rewrite + rerank add two Gemini Flash calls per message (~1–2s latency) — toggle off via env if needed. Reranking/rewrite LLM calls reuse the proven `generateText`+parse+fallback pattern (like `classify`).

**Whole-document mode (RAG Phase 3)** — top-k chunk retrieval structurally can't answer set-wide questions ("list every storm sheet") no matter how large k is. The `read_document` tool (`src/lib/documents/tool.ts`, `createReadDocumentTool`) lets Claude read a project document's full extracted text window-by-window instead: source is `extracted.txt` in Storage (`documents/<projectId>/<docId>/[revN/]extracted.txt`, persisted since RAG Phase 1); windowing (`src/lib/documents/windowing.ts` `sliceWindow`) cuts on `# Page n` anchors when present (vision/hybrid docs emit them) or falls back to character-offset windows, capped at `READ_DOC_WINDOW_CHARS` (default `100000` chars ≈ 25k tokens) per call, with a continuation pointer (`nextOffset`/`fromPage`) when more remains. Wired into `/api/chat` for Claude project chats with ≥1 `ready` document alongside a compact `[Project documents]` manifest (id/filename/pageCount/charCount/extractionMethod/extractionPartial — one DB query) and `READ_DOCUMENT_GUIDANCE`: chunks-first for targeted questions, `read_document` for set-wide/exhaustive asks or when chunks are visibly insufficient. Missing `extracted.txt` (pre-Phase-1 docs) or Storage errors degrade to an in-band tool error, never a throw. Sweep budget is the existing `stopWhen: stepCountIs(12)`. **Tool outputs are not persisted** — `saveMessage` writes assistant TEXT only, so `read_document` window text never reaches the `messages` table (no stubbing needed; reload shows the answer without a tool card, same as `web_search`). An active `read_document` tool part drives the **`reading-documents`** chat stage (`src/lib/chatStage.ts`) → `ThinkingStatus` "Reading documents…".

**Vision extraction (Phase C2, reworked in RAG Phase 2 v4.45.0)** — `src/lib/visionExtraction.ts` handles two paths: `extractViaVision(buffer)` splits the PDF into page-range segments via `src/lib/pdfSegments.ts` (**pdf-lib**; default 20 pages/segment, each kept under Gemini's 50MB-per-request PDF cap with recursive halving) and sends each segment **inline as a native PDF** (`file` part) in one Gemini call — bounded concurrency (`src/lib/concurrency.ts`) + retry; Gemini reads the embedded text layer natively and sees each page as an image (no per-page rasterizing; `pdfjs-dist`/`@napi-rs/canvas` remain only for thumbnails). `extractViaVisionImage(buffer, mimeType)` vision-extracts a single image. Both degrade to `''` if no Gemini key. The vision fallback triggers on thin text **absolute** (`EXTRACTION_MIN_TEXT_CHARS`, default `100`) **or per page** (`EXTRACTION_MIN_CHARS_PER_PAGE`, default `200` — catches CAD plan sets whose only text layer is title blocks). Env knobs: `EXTRACTION_MODEL` (default `gemini-3.5-flash`), `EXTRACTION_MAX_PAGES` (default `500`), `EXTRACTION_SEGMENT_PAGES` (default `20`), `EXTRACTION_SEGMENT_CONCURRENCY` (default `2`), `EXTRACTION_SEGMENT_MAX_BYTES` (default 45MB), `EXTRACTION_MAX_OUTPUT_TOKENS` (default `60000`; `finishReason==='length'` flags the doc partial), `EXTRACTION_SEGMENT_RETRIES` (default `2`). A failed segment, skipped oversize page, output truncation, or page-capping all surface as `extraction_partial` — never silent. Once text is extracted the downstream chunk → embed → pgvector RAG pipeline is unchanged. **Per-page hybrid splice (v4.46.0)** — a document-level density gate can't see a text-rich plan set with individually sparse sheets (AutoCAD SHX notes sheets: stroke-geometry text, title-block-only text layer). On the text path, pages whose own text layer falls under `EXTRACTION_HYBRID_PAGE_MIN_CHARS` (default `500`) are grouped into contiguous runs and vision-extracted via `extractPagesViaVision`/`splitPdfPageRuns` (reusing the v4.45.0 segment machinery), then spliced back into the document text in page order; more sparse pages than `EXTRACTION_HYBRID_MAX_PAGES` (default `80`) skips hybrid entirely (the whole-doc gate owns pervasively-sparse docs). `extraction_method` becomes `'hybrid'` (a `hybrid` chip in `DocumentCard`, alongside `vision`); the splice is best-effort and never fatal — a hybrid failure just keeps the plain text. **Failed-page tracking + provenance tags (RAG Phase 3, v4.48.0)** — a segment that fails after retries, or an oversize page that's skipped, records its absolute page range into `ExtractionResult.failedPages`, plumbed via `/api/documents/process` → `documents.failed_pages` (jsonb) → `GET /api/documents` → `DocumentCard`'s Partial tooltip ("Vision failed on pages 12–14, 30") and `read_document`'s `unavailablePages`. Separately, every vision-derived page run gets a `[pages 12–14 · vision]` header (`visionRunHeader` in `src/lib/visionExtraction.ts`) prepended to its text before chunking — in `extractViaVision`'s joined output and in the hybrid splice — so Claude can hedge on OCR-derived content; no schema change, no backfill.
...
- **UI persistence**: `useLocalStorage` hook with deferred hydration (reads in `useEffect` to avoid SSR mismatch). Instances sharing a key sync **cross-tab** (`storage` event) and, since v4.51.0, **within the same tab** via a `local-storage` CustomEvent broadcast on write — a last-seen-raw ref breaks echo loops on object values (don't remove it; objects never bail on `Object.is`).
