...
- **Migrations** (versioned, in `drizzle/`): `npx drizzle-kit generate` to author, `DIRECT_URL=... npx drizzle-kit migrate` to apply. `drizzle.config.ts` uses `dialect: "postgresql"`. (Legacy `npx drizzle-kit push` is no longer the workflow.) Applied to Supabase: `0000`–`0014` (latest applied: `0013` RLS + indexes, `0014` extraction fidelity columns). **`0015`/`0016` (RAG Phase 3 — `failed_pages`, `pg_trgm`/`content_tsv` hybrid search) are authored but NOT YET applied — this is MANDATORY, not advisory: run `DIRECT_URL=... npx drizzle-kit migrate` BEFORE pushing/deploying this release.** The deployed code's `documents.failed_pages` column reference means `GET/POST /api/documents*` — document listing, uploads, the Files panel — breaks app-wide against an unmigrated DB (missing-column error), not just hybrid retrieval or failed-page reporting.
