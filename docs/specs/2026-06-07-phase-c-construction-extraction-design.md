...
- **PDF → image:** prefer `unpdf` `renderPageAsImage` (already a dependency, serverless-friendly). **Verify this API exists/works server-side in the spike.** Fallback: client-side pdf.js.
