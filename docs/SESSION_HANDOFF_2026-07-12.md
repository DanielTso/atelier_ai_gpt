...
- **CSP PDF-preview fix (deployed d7d7939)**: Chromium's post-OOPIF PDF viewer subjects iframe'd PDFs to the EMBEDDER's object-src; the hardening-era 'none' blocked the artifact PDF preview ("This content is blocked"). Now object-src 'self' — verified live on prod headers.
