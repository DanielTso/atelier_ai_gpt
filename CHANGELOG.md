...
- **Dynamic model registry** (`src/lib/models/`). `getModelRegistry()` fetches the live Anthropic catalog (`GET /v1/models`, paginated, 5s timeout), normalizes and prices every model, and degrades to a hand-authored `STATIC_SEED` on any fetch failure — the picker is never empty. Cached with a TTL (5 min live / 60s degraded, so a transient outage self-heals fast). `curateCatalog()` reduces the full catalog to the single newest entry per family — preferring a non-dated-snapshot id, but falling back to a dated one when a family has no undated entry at all, which is true TODAY for Haiku 4.5, Sonnet 4.5, Opus 4.5, and Opus 4.1 in the live catalog — ordered `opus`/`fable`/`sonnet`/`haiku`, unrecognized families sorted last. That curated list is what `GET /api/models` now serves, replacing the old static array.
- **Alias-vs-dated id indexing.** The live catalog often ships a family with only a dated snapshot id (e.g. `claude-haiku-4-5-20251001`, no bare `claude-haiku-4-5`), while the app routes by the bare alias everywhere. `registry.byId` now indexes both forms, and `resolveRequestedModel()`/`getModelCapabilities()` never throw — an unknown/stale/retired id degrades to the current default with a `console.warn` instead of a 400. Fixes a real bug where a stale `projects.default_model` 400'd the chat with no recovery, and closes a silent-5×-cost landmine where a cheap-tier request could resolve to Opus.
- **Live pricing with price badges.** `src/lib/models/pricing.ts` resolves each model's per-MTok rate in order: a DB override (`settings` key `model-pricing-overrides`, validated per-entry, malformed rates dropped rather than corrupting cost data) → an exact-id table → a family-tier estimate (`estimated: true`) → a conservative Opus-tier default. The model picker shows each model's rate (`~$X / $Y` with an "est." chip when inferred); Nano Banana 2 shows "per image" instead of a token rate.
- **Capability-driven reasoning effort.** `Effort` is now one shared union (`src/types.ts`): `low | medium | high | xhigh | max` — `xhigh` was previously undeclared in the UI despite Opus 5/Sonnet 5/Fable 5 supporting it. Which levels a given model actually accepts comes from its registry `capabilities.effortLevels` (derived from Anthropic's live per-level `capabilities` tree), not a name-prefix guess — `createProvider()` only sends `effort` when the resolved model reports that level, and the effort pill only offers levels the selected model supports.
- **Persona tiers.** 13 of the 14 built-in personas now set `model` to a tier (`flagship | opus | sonnet | haiku`) instead of an exact id, resolved client-side against the live `models` list — they automatically adopt Anthropic's newest release in that family (`flagship` → the `fable` family). **Contract Abstract stays pinned to the exact `claude-fable-5`**: its locked 22-field extraction schema must not shift under a silent model swap. A persona chip always resolves to the catalog's real display name before rendering, even for a dated-only id.
- **Cost visibility** (spec §C6). New `usage_events` table (migration `0018`) records one best-effort row per LLM generation — chat, artifact-regenerate, summarize, generate-title, classify, memory-suggest — via `recordUsage()` (`src/lib/usage.ts`). `cost_usd` is computed and **frozen at write time**; a later price change never reprices history. Settings → new **Usage** tab shows monthly spend by model for the trailing 3 months; the chat context menu shows a per-chat cost figure, lazily fetched on menu open.

### Fixed

- **Cache-token double-counting.** Verified against the installed `@ai-sdk/anthropic@3.0.98` source (not docs): the AI SDK's `usage.inputTokens` is the *inclusive* sum of fresh + cache-read + cache-write input, even though Anthropic's raw API reports the three as mutually exclusive. The naive formula (`inputTokens × rate + cacheRead × 0.1 × rate + …`) would have double-billed every cached token, worst on exactly the long-context project chats this app is built for. `usageTokens()` derives the fresh count from `usage.inputTokenDetails.noCacheTokens` directly; `usage_events.input_tokens` stores fresh tokens only, and the three input columns are mutually exclusive so rollups can sum them without double-counting.
- **Unpriced rows never read as "free."** The internal `gemini-3.5-flash` housekeeping model (title/summarize/classify/memory-suggest) is deliberately not token-priced — its rows write `cost_usd=0, cost_estimated=true`, the same tuple a genuinely free Claude generation could produce. The Usage tab renders that tuple as "unpriced," never "$0.00." `formatUsd()` also shows up to 8 decimals for sub-cent costs so a real $0.00003 charge is never indistinguishable from zero (a plain `toFixed(2)` would round both to "$0.00").
- Provider 400s were previously masked as a generic "An error occurred." (a `streamText` synchronous-error gotcha carried over from earlier in this feature) — confirmed still correctly routed via `onError`, not the route's `try/catch`.
